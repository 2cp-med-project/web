...
function FileRow({
  ...
  return (
    <Link
      to="/d/patients/$patientId/files/$fileId"
      params={{ patientId, fileId: file.id }}
...
export function FilesTableContent({
  patientId,
  files,
}: FilesTableContentProps) {
  return (
    <section className="overflow-hidden rounded-sm border border-[#d7ece5] bg-white">
      ...
      {files.length > 0 ? (
        files.map((file) => (
          <FileRow key={file.id} patientId={patientId} file={file} />
        ))
      ) : (
