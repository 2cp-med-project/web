...
export const fetchPage = (pagination: PaginationAttributes) => {
  return new Promise<Page<Patient>>((res) => {
    setTimeout(() => {
      ...
      const filteredPatients = AdminData.Patients.patients.filter((p) =>
        p.fullname.toLowerCase().includes(pagination.search.trim().toLowerCase()),
      );
      const patients = filteredPatients.slice(start, end);
      const nextPage =
        patients.length === filteredPatients.length ? null : pagination.page + 1;
      const count = filteredPatients.length;
      return res({ data: patients, nextPage, count });
    }, 400);
  });
};
...
export const fetchOne = (id: string) => {
  return new Promise<PatientDetails | null>((res, rej) =>
    setTimeout(() => {
      const patient = AdminData.Patients.patientsWithDetails.find(
        (p) => p.id === id,
      );
...
export const deleteOne = (id: string) => {
  return new Promise<void>((res, rej) =>
    setTimeout(() => {
      const index = AdminData.Patients.patients.findIndex((p) => p.id === id);
      if (index === -1) return rej(new PatientNotFoundError(id));
      AdminData.Patients.patients.splice(index, 1);
      return res();
    }, 500),
  );
};